...
const S = `
@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:ital,wght@0,400;0,600;1,400&family=Outfit:wght@300;400;500;600;700&family=Syne:wght@400;600;700;800;900&display=swap');
...
:root{
  ...
  --s1:#0C0A18;
  --s2:#120F20;
  --bd:rgba(140,90,255,.14);
  --bd2:rgba(140,90,255,.22);
  --tx:#F0ECFF;
  --mu:#6B5F9E;
  --ac:#9D4EFF;
  --ac2:#7B2FFF;
...
/* ══ THEMES ══ */
.nord{
  --bg:#1E2130;--s1:#252A3D;--s2:#2D3350;
  --bd:rgba(136,192,208,.15);--bd2:rgba(136,192,208,.25);
  --tx:#ECEFF4;--mu:#7B88A1;
  --ac:#88C0D0;--ac2:#5E9FB0;--pink:#B48EAD;--cyan:#81A1C1;
  --green:#A3BE8C;--red:#BF616A;--orange:#D08770;--blue:#81A1C1;
}
.dracula{
  --bg:#191A26;--s1:#21222C;--s2:#2A2B3D;
  --bd:rgba(255,121,198,.12);--bd2:rgba(255,121,198,.22);
  --tx:#F8F8F2;--mu:#6272A4;
  --ac:#FF79C6;--ac2:#CC5FAA;--pink:#FF79C6;--cyan:#8BE9FD;
  --green:#50FA7B;--red:#FF5555;--orange:#FFB86C;--blue:#8BE9FD;
}
.light{
  --bg:#F4F2FF;--s1:#FFFFFF;--s2:#EDE9FF;
  --bd:rgba(100,50,200,.12);--bd2:rgba(100,50,200,.22);
  --tx:#1A1030;--mu:#8070B0;
  --ac:#7B2FFF;--ac2:#6020EE;--pink:#D0208A;--cyan:#0090CC;
  --green:#007744;--red:#CC0030;--orange:#BB5500;--blue:#0070CC;
}
.noir{
  --bg:#000000;--s1:#0A0A0A;--s2:#111111;
  --bd:rgba(255,255,255,.07);--bd2:rgba(255,255,255,.13);
  --tx:#F5F5F5;--mu:#555555;
  --ac:#E0E0E0;--ac2:#BBBBBB;
  --pink:#FF3CAC;--cyan:#00D4FF;
  --green:#00FF88;--red:#FF4D6A;--orange:#FF8C42;--blue:#00D4FF;--gold:#FFB700;
}
...
body{
  ...
  background-image:
    radial-gradient(ellipse 80% 60% at 10% 0%,rgba(80,20,200,.25) 0%,transparent 55%),
    radial-gradient(ellipse 60% 40% at 90% 100%,rgba(255,60,172,.12) 0%,transparent 50%);
...
body::before{
  ...
  background-image:linear-gradient(rgba(140,90,255,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(140,90,255,.04) 1px,transparent 1px);
...
::-webkit-scrollbar-thumb{background:rgba(157,78,255,.3);border-radius:2px}
::-webkit-scrollbar-thumb:hover{background:rgba(157,78,255,.5)}
...
.nt{
  ...
  background:rgba(12,10,24,.8);
  cursor:pointer;transition:all .22s cubic-bezier(.4,0,.2,1);
  flex-shrink:0;overflow:hidden;
  box-shadow:0 2px 8px rgba(0,0,0,.3);
}
.nt:hover{border-color:var(--bd2);transform:translateY(-3px);box-shadow:0 8px 24px rgba(0,0,0,.5)}
.nt.on{border-width:2px;box-shadow:0 0 24px rgba(157,78,255,.3)}
.nt-img{
...
.nt:hover .nt-img{transform:scale(1.04)}
.nt-body{
  padding:8px 9px 9px;
  display:flex;flex-direction:column;gap:2px;
}
.nt-name{font-size:10px;font-weight:700;color:var(--tx);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-family:var(--font-ui);line-height:1.2}
.nt-cat{font-size:7px;color:var(--mu);font-family:var(--font-mono);white-space:nowrap;letter-spacing:.4px;text-transform:uppercase}
.nt-desc{font-size:8px;color:rgba(160,150,200,.5);line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;font-family:var(--font-ui);margin-top:2px}
.nt.on .nt-name{color:#fff}
.nt.on .nt-cat{color:rgba(200,180,255,.7)}
.nt.on .nt-desc{color:rgba(200,180,255,.4)}

/* ══ PILLS (model toggles) ══ */
.pills{display:flex;gap:4px;margin-left:auto;flex-wrap:wrap;align-items:center}
.pill{
  display:flex;align-items:center;gap:4px;
  padding:clamp(3px,.7vw,5px) clamp(6px,1.2vw,10px);
  border-radius:6px;border:1px solid;
  font-family:var(--font-mono);font-size:clamp(8px,1.3vw,10px);
  cursor:pointer;transition:all .2s;background:transparent;white-space:nowrap;
}
.pill:hover{filter:brightness(1.3);transform:translateY(-1px)}
.pill.on{box-shadow:0 0 12px currentColor}
.pfree{font-size:7px;padding:1px 4px;border-radius:2px;background:rgba(0,255,136,.12);color:var(--green);font-weight:700}
.btn-xs{background:transparent;border:1px solid var(--bd);color:var(--mu);padding:3px 8px;border-radius:5px;font-size:9px;font-family:var(--font-mono);cursor:pointer;transition:all .15s}
.btn-xs:hover{border-color:var(--red);color:var(--red)}
.theme-btn{background:rgba(12,10,24,.7);border:1px solid var(--bd);color:var(--mu);border-radius:7px;width:30px;height:30px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:all .2s;flex-shrink:0;padding:0}
.theme-btn:hover{border-color:var(--bd2);color:var(--tx);background:rgba(157,78,255,.08)}
.voice-btn{background:rgba(12,10,24,.7);border:1px solid var(--bd);color:var(--mu);border-radius:7px;width:30px;height:30px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:all .2s;flex-shrink:0;padding:0}
.voice-btn:hover{border-color:var(--bd2);color:var(--tx);background:rgba(157,78,255,.08)}
.voice-btn.speaking{border-color:rgba(0,212,255,.4);color:var(--cyan);box-shadow:0 0 10px rgba(0,212,255,.2)}
...
.tbar-lbl{color:var(--ac);font-weight:700;letter-spacing:1.5px;font-size:8px;font-family:var(--font-mono);text-transform:uppercase}
.ti{display:flex;align-items:center;gap:4px;flex-shrink:0}
.tr{width:clamp(28px,4.5vw,50px);height:2px;background:rgba(157,78,255,.12);border-radius:2px;overflow:hidden}
.tf{height:100%;border-radius:2px;transition:width .5s;box-shadow:0 0 6px currentColor}
.tbar-total{margin-left:auto;white-space:nowrap;font-family:var(--font-mono)}
...
.col.off{opacity:.08;filter:grayscale(1);pointer-events:none}
.col.solo-dim{opacity:.08;filter:grayscale(1);pointer-events:none}
...
.hist-folder-tabs{display:flex;gap:2px;padding:6px 8px;flex-wrap:wrap;border-bottom:1px solid var(--bd);flex-shrink:0;background:rgba(12,10,24,.5)}
.hist-folder-tab{padding:3px 8px;border-radius:4px;border:1px solid transparent;font-size:8px;cursor:pointer;color:var(--mu);background:transparent;font-family:var(--font-mono);white-space:nowrap;transition:all .15s;max-width:80px;overflow:hidden;text-overflow:ellipsis}
.hist-folder-tab:hover{background:rgba(157,78,255,.08);color:var(--tx)}
.hist-folder-tab.active{background:rgba(157,78,255,.12);border-color:rgba(157,78,255,.3);color:var(--ac)}
.hist-folder-add{padding:2px 6px;border-radius:4px;border:1px dashed rgba(157,78,255,.25);font-size:8px;cursor:pointer;color:var(--mu);background:transparent;font-family:var(--font-mono);transition:all .15s}
.hist-folder-add:hover{border-color:var(--ac);color:var(--ac)}
.hist-hdr{padding:10px 12px;border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:6px;flex-shrink:0}
.hist-hdr-title{font-family:var(--font-display);font-weight:700;font-size:11px;color:var(--tx);flex:1;white-space:nowrap;overflow:hidden}
.hist-new-btn{
  background:rgba(157,78,255,.12);border:1px solid rgba(157,78,255,.3);
  border-radius:5px;color:var(--ac);font-size:9px;font-weight:700;
  cursor:pointer;padding:3px 8px;font-family:var(--font-mono);
  white-space:nowrap;transition:all .15s;
}
.hist-new-btn:hover{background:rgba(157,78,255,.22);box-shadow:0 0 12px rgba(157,78,255,.2)}
.hist-save-btn{
  background:rgba(0,255,136,.06);border:1px solid rgba(0,255,136,.25);
  border-radius:5px;color:var(--green);font-size:9px;font-weight:700;
  cursor:pointer;padding:3px 8px;font-family:var(--font-mono);white-space:nowrap;transition:all .15s;
}
.hist-save-btn:hover{background:rgba(0,255,136,.14)}
...
.hist-empty{padding:24px 14px;text-align:center;font-size:9px;color:var(--mu);line-height:1.8}
.hist-item{padding:10px 12px;border-bottom:1px solid var(--bd);cursor:pointer;transition:background .15s;display:flex;flex-direction:column;gap:3px;position:relative}
.hist-item:hover{background:rgba(157,78,255,.05)}
.hist-item.active{background:rgba(157,78,255,.08);border-left:2px solid var(--ac)}
.hist-item-title{font-size:10px;font-weight:600;color:var(--tx);line-height:1.35;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;padding-right:18px}
...
.hist-item-del{position:absolute;top:8px;right:8px;background:none;border:none;color:var(--mu);cursor:pointer;font-size:11px;padding:2px 4px;border-radius:3px;opacity:0;transition:opacity .15s}
...
.hist-toggle{
  background:rgba(5,3,8,.7);border:none;border-right:1px solid var(--bd);
  color:var(--mu);cursor:pointer;font-size:10px;padding:0 5px;
  writing-mode:vertical-lr;letter-spacing:1px;
  transition:background .15s;flex-shrink:0;
  display:flex;align-items:center;justify-content:center;min-height:40px;
}
.hist-toggle:hover{background:rgba(157,78,255,.08);color:var(--tx)}
...
.ch-btn{
  ...
  color:var(--mu);cursor:pointer;font-size:10px;padding:2px 5px;
  transition:all .15s;line-height:1;font-family:var(--font-mono);
}
.ch-btn:hover{border-color:var(--bd2);color:var(--tx);background:rgba(157,78,255,.07)}
.ch-btn.solo-on{border-color:var(--ac);color:var(--ac);background:rgba(157,78,255,.12);box-shadow:0 0 8px rgba(157,78,255,.15)}
.ci{font-size:clamp(11px,1.9vw,14px)}
.cn{font-family:var(--font-display);font-weight:700;font-size:clamp(9px,1.5vw,11px);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.csub{font-size:7px;color:var(--mu);font-family:var(--font-mono)}
.cm{margin-left:auto;display:flex;align-items:center;gap:4px;flex-shrink:0;margin-right:4px}
.mt{font-size:7px;color:var(--mu);white-space:nowrap;font-family:var(--font-mono)}
...
.msg.u{
  background:rgba(157,78,255,.06);
  color:rgba(200,190,240,.7);
  font-style:italic;
  border-color:rgba(157,78,255,.15);
}
.msg.u::before{content:'> ';color:var(--ac);opacity:.6}
.msg.a{
...
.msg.ld{color:var(--mu);background:rgba(12,10,24,.5);border-color:var(--bd)}
.msg.blocked{
  color:var(--orange);
...
.scroll-to-bottom{
  ...
  background:rgba(157,78,255,.2);border:1px solid rgba(157,78,255,.4);
  border-radius:50%;width:28px;height:28px;
  display:flex;align-items:center;justify-content:center;
  cursor:pointer;font-size:12px;color:var(--ac);z-index:20;
  backdrop-filter:blur(6px);transition:all .2s;
  box-shadow:0 4px 16px rgba(0,0,0,.4);
}
.scroll-to-bottom:hover{background:rgba(157,78,255,.35);box-shadow:0 0 16px rgba(157,78,255,.3)}

/* ══ LOADING DOTS ══ */
.dots span{animation:blink 1.2s infinite;font-size:16px;line-height:0}
.dots span:nth-child(2){animation-delay:.2s}
.dots span:nth-child(3){animation-delay:.4s}
@keyframes blink{0%,80%,100%{opacity:.15}40%{opacity:1}}
.empty{color:var(--mu);font-size:9px;text-align:center;margin-top:clamp(16px,3vh,32px);font-style:italic;font-family:var(--font-mono)}

/* ══ STREAMING CURSOR ══ */
@keyframes blinkCursor{0%,100%{opacity:1}50%{opacity:0}}
.streaming-cursor::after{content:"▋";display:inline-block;animation:blinkCursor .7s step-end infinite;color:var(--ac);margin-left:1px;font-size:.9em}
...
textarea{
  ...
  padding:clamp(7px,1.2vw,10px) clamp(9px,1.8vw,12px);
  resize:none;outline:none;line-height:1.5;max-height:110px;
  transition:border-color .2s,box-shadow .2s;
  backdrop-filter:blur(4px);
}
textarea:focus{border-color:rgba(157,78,255,.5);box-shadow:0 0 0 3px rgba(157,78,255,.08),0 0 16px rgba(157,78,255,.08)}
textarea::placeholder{color:var(--mu)}
.foot-btns{display:flex;gap:6px;align-items:flex-end}
.sbtn{
  background:linear-gradient(135deg,var(--ac2),var(--pink));
  color:#fff;border:none;border-radius:8px;
  width:clamp(32px,4.5vw,40px);height:clamp(32px,4.5vw,40px);
  cursor:pointer;font-size:15px;font-weight:bold;
  display:flex;align-items:center;justify-content:center;
  transition:all .2s;flex-shrink:0;
  box-shadow:0 0 20px rgba(123,47,255,.3);
}
.sbtn:hover{transform:scale(1.06);box-shadow:0 0 30px rgba(123,47,255,.5)}
.sbtn:disabled{opacity:.3;cursor:not-allowed;transform:none;box-shadow:none}
.gbtn{
  background:rgba(12,10,24,.8);border:1px solid var(--bd);color:var(--mu);
  border-radius:8px;width:clamp(32px,4.5vw,40px);height:clamp(32px,4.5vw,40px);
  cursor:pointer;font-size:14px;
  display:flex;align-items:center;justify-content:center;
  transition:all .2s;flex-shrink:0;position:relative;
}
.gbtn:hover{border-color:var(--green);color:var(--green);box-shadow:0 0 12px rgba(0,255,136,.15)}
.gbtn.loading{animation:glow .8s infinite alternate}
@keyframes glow{from{box-shadow:none}to{box-shadow:0 0 12px var(--ac)}}

/* ══ GRAMMAR POPUP ══ */
.grammar-popup{
  position:absolute;bottom:calc(100% + 10px);left:0;right:0;
  background:rgba(12,10,24,.95);border:1px solid rgba(157,78,255,.3);
  border-radius:10px;padding:12px;z-index:50;font-size:10px;min-width:200px;
  backdrop-filter:blur(16px);box-shadow:0 20px 60px rgba(0,0,0,.5),0 0 20px rgba(157,78,255,.1);
}
.gp-title{color:var(--ac);font-weight:600;font-size:8px;letter-spacing:1px;margin-bottom:8px;font-family:var(--font-mono);text-transform:uppercase}
.gp-orig{color:var(--mu);text-decoration:line-through;font-size:9px;margin-bottom:4px}
.gp-corr{color:var(--green);font-size:10px}
.gp-btns{display:flex;gap:6px;margin-top:10px}
.gp-btn{flex:1;padding:5px 8px;border-radius:5px;font-size:9px;font-family:var(--font-mono);cursor:pointer;border:1px solid var(--bd);background:transparent;color:var(--mu);transition:all .15s}
.gp-btn:hover{border-color:var(--bd2);color:var(--tx)}
.gp-btn.apply{background:linear-gradient(135deg,var(--ac2),var(--pink));border-color:transparent;color:#fff;font-weight:600}
.fhint{margin-top:4px;font-size:clamp(7px,1.3vw,9px);color:var(--mu);text-align:center;font-family:var(--font-mono)}

/* ══ COUNTDOWN ══ */
.countdown{
  font-size:8px;color:var(--red);white-space:nowrap;font-family:var(--font-mono);
...
@media(max-width:768px){
  ...
  .mobile-col-tabs{
    display:flex !important;padding:4px 10px;gap:4px;
    overflow-x:auto;border-bottom:1px solid var(--bd);
    flex-shrink:0;background:rgba(5,3,8,.8);scrollbar-width:none;
  }
  .mobile-col-tabs::-webkit-scrollbar{display:none}
  .mct-btn{
    padding:4px 12px;border-radius:5px;border:1px solid var(--bd);
    font-size:10px;font-family:var(--font-mono);cursor:pointer;
    background:transparent;color:var(--mu);white-space:nowrap;flex-shrink:0;transition:all .15s;
  }
  .mct-btn.active{background:linear-gradient(135deg,rgba(123,47,255,.3),rgba(255,60,172,.15));color:#fff;border-color:rgba(157,78,255,.4);font-weight:600}
  .nav{padding:5px 10px}
}

/* ══ SCROLL TAB ══ */
.scroll-tab{flex:1;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.pad{padding:clamp(12px,2vw,20px)}

/* ══ WEB IAS ══ */
.web-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(clamp(160px,20vw,220px),1fr));gap:clamp(8px,1.5vw,12px)}
.web-card{
  background:rgba(12,10,24,.7);border:1px solid var(--bd);border-radius:12px;
  padding:clamp(12px,2vw,16px);display:flex;flex-direction:column;gap:10px;
  transition:all .25s;backdrop-filter:blur(6px);
}
.web-card:hover{transform:translateY(-3px);border-color:var(--bd2);box-shadow:0 16px 48px rgba(0,0,0,.4),0 0 20px rgba(157,78,255,.06)}
.wc-hdr{display:flex;align-items:center;gap:9px}
.wc-icon{width:clamp(28px,3.5vw,36px);height:clamp(28px,3.5vw,36px);border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:clamp(12px,1.8vw,16px);font-weight:700;flex-shrink:0}
.wc-name{font-family:var(--font-display);font-weight:700;font-size:clamp(11px,1.8vw,13px)}
.wc-sub{font-size:8px;color:var(--mu)}
.wc-desc{font-size:clamp(9px,1.5vw,11px);color:var(--mu);line-height:1.55;flex:1}
.wc-btn{
  display:flex;align-items:center;justify-content:center;gap:5px;
  padding:7px;border-radius:7px;border:1px solid;
  font-family:var(--font-mono);font-size:10px;font-weight:600;
  cursor:pointer;background:transparent;text-decoration:none;transition:all .2s;
}
...
.debate-intro{padding:clamp(16px,3.5vw,28px);display:flex;flex-direction:column;align-items:center;gap:14px;flex:1;justify-content:center;text-align:center}
.debate-title{font-family:var(--font-display);font-size:clamp(16px,3vw,22px);font-weight:800;background:linear-gradient(135deg,var(--ac),var(--pink));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.phase-header{padding:9px clamp(12px,2vw,18px);display:flex;align-items:center;gap:8px;cursor:pointer;background:rgba(12,10,24,.5);user-select:none;transition:background .15s}
.phase-header:hover{background:rgba(157,78,255,.05)}
.ph-badge{padding:2px 7px;border-radius:4px;font-size:7px;font-weight:600;letter-spacing:1px}
.ph-title{font-family:var(--font-display);font-size:clamp(10px,1.8vw,12px);font-weight:700}
.ph-chev{margin-left:auto;color:var(--mu);transition:transform .2s}
.ph-chev.open{transform:rotate(180deg)}
.phase-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(clamp(180px,26vw,260px),1fr));border-top:1px solid var(--bd)}
.pg-cell{padding:clamp(8px,1.5vw,12px);border-right:1px solid var(--bd);border-bottom:1px solid var(--bd)}
...
.syn-block{margin:clamp(8px,1.5vw,14px);border:1px solid rgba(157,78,255,.25);border-radius:12px;overflow:hidden;flex-shrink:0}
.syn-hdr{padding:10px clamp(12px,2vw,16px);background:linear-gradient(90deg,rgba(123,47,255,.12),rgba(5,3,8,.8));display:flex;align-items:center;gap:8px;border-bottom:1px solid var(--bd)}
.syn-title{font-family:var(--font-display);font-size:clamp(10px,1.8vw,13px);font-weight:800;background:linear-gradient(135deg,var(--ac),var(--pink));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.syn-by{font-size:9px;color:var(--mu);margin-left:auto}
.syn-body{padding:clamp(10px,1.8vw,14px);font-size:clamp(10px,1.7vw,12px);line-height:1.72;color:var(--tx);background:rgba(12,10,24,.6)}
.syn-body.mu{color:var(--mu);font-style:italic}
.debate-foot{padding:clamp(8px,1.5vw,12px) clamp(10px,2vw,16px);border-top:1px solid var(--bd);background:rgba(5,3,8,.7);flex-shrink:0}

/* ══ BUTTONS ══ */
.launch-btn{
  background:linear-gradient(135deg,var(--ac2),var(--pink));
  color:#fff;border:none;border-radius:8px;
  padding:0 clamp(12px,2.2vw,20px);height:clamp(32px,4.5vw,40px);
  font-family:var(--font-mono);font-size:clamp(10px,1.6vw,11px);font-weight:600;
  cursor:pointer;transition:all .2s;white-space:nowrap;flex-shrink:0;
  box-shadow:0 0 20px rgba(123,47,255,.3);
}
.launch-btn:hover{box-shadow:0 0 35px rgba(123,47,255,.5);transform:translateY(-1px)}
.launch-btn:disabled{opacity:.3;cursor:not-allowed;box-shadow:none;transform:none}
.prog-bar{height:2px;background:rgba(157,78,255,.12);border-radius:2px;overflow:hidden;margin:6px clamp(10px,2vw,16px) 0}
.prog-fill{height:100%;background:linear-gradient(90deg,var(--ac),var(--pink));border-radius:2px;transition:width .5s;box-shadow:0 0 8px var(--ac)}
.prog-lbl{padding:3px clamp(10px,2vw,16px) 6px;font-size:8px;color:var(--mu);font-family:var(--font-mono)}

/* ══ CONFIG ══ */
.cfg-wrap{flex:1;overflow-y:auto;padding:clamp(14px,2.2vw,20px);scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.sec-title{
  font-family:var(--font-display);font-size:clamp(11px,1.8vw,14px);font-weight:800;
...
.tbl{width:100%;border-collapse:collapse;border:1px solid var(--bd);border-radius:10px;overflow:hidden;min-width:500px}
.tbl th{padding:6px 10px;background:rgba(12,10,24,.8);font-size:8px;color:var(--mu);text-align:left;font-weight:600;letter-spacing:1px;border-bottom:1px solid var(--bd);font-family:var(--font-mono);text-transform:uppercase}
.tbl td{padding:8px 10px;border-bottom:1px solid var(--bd);font-size:clamp(9px,1.6vw,11px);vertical-align:middle}
.tbl tr:last-child td{border-bottom:none}
.tbl tr:hover td{background:rgba(157,78,255,.04)}
.sbadge{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:4px;font-size:8px;font-weight:600}
.free-badge{display:inline-flex;padding:1px 5px;border-radius:3px;font-size:7px;font-weight:700;background:rgba(0,255,136,.1);color:var(--green);margin-left:3px;border:1px solid rgba(0,255,136,.2)}
.key-row{display:flex;gap:5px;align-items:center}
.key-inp{
  flex:1;background:rgba(5,3,8,.8);border:1px solid var(--bd);border-radius:6px;
  color:var(--tx);font-family:var(--font-mono);font-size:10px;padding:6px 8px;
  outline:none;min-width:0;transition:border-color .2s,box-shadow .2s;
}
.key-inp:focus{border-color:rgba(157,78,255,.5);box-shadow:0 0 0 2px rgba(157,78,255,.08)}
.key-inp::placeholder{color:var(--mu)}
.key-link{
  color:var(--ac);text-decoration:none;font-size:8px;white-space:nowrap;
  border:1px solid var(--bd);padding:4px 7px;border-radius:5px;
  font-family:var(--font-mono);transition:all .15s;
}
.key-link:hover{border-color:var(--ac);box-shadow:0 0 8px rgba(157,78,255,.15)}
.save-btn{
  background:linear-gradient(135deg,var(--ac2),var(--pink));color:#fff;border:none;
  border-radius:5px;padding:5px 9px;font-family:var(--font-mono);font-size:8px;font-weight:600;cursor:pointer;white-space:nowrap;
}
.save-btn:disabled{opacity:.3;cursor:not-allowed}
.file-btns{display:flex;gap:8px;flex-wrap:wrap}
.fbtn{
  display:flex;align-items:center;gap:6px;
  padding:clamp(7px,1.2vw,10px) clamp(10px,1.8vw,15px);
  border-radius:8px;font-family:var(--font-mono);font-size:clamp(9px,1.6vw,11px);
  cursor:pointer;transition:all .2s;border:1px solid var(--bd);
  background:rgba(12,10,24,.7);color:var(--tx);
}
.fbtn:hover{border-color:var(--bd2);background:rgba(157,78,255,.06);transform:translateY(-1px)}
.fbtn.p{
  background:linear-gradient(135deg,rgba(123,47,255,.2),rgba(255,60,172,.1));
  color:var(--tx);border-color:rgba(157,78,255,.3);font-weight:600;
}
.fbtn.p:hover{background:linear-gradient(135deg,rgba(123,47,255,.3),rgba(255,60,172,.2));box-shadow:0 0 20px rgba(123,47,255,.2)}
.cfg-note{
  font-size:clamp(9px,1.4vw,11px);color:var(--mu);line-height:1.65;margin-top:8px;
  padding:10px 12px;background:rgba(12,10,24,.6);border-radius:8px;border:1px solid var(--bd);
}
.cfg-note code{color:var(--ac);background:rgba(157,78,255,.08);padding:1px 5px;border-radius:3px;font-size:10px;font-family:var(--font-mono)}

/* ══ CODE BLOCK ══ */
.md-code-block{
  background:rgba(5,3,8,.9);border:1px solid var(--bd);
  border-radius:10px;overflow:hidden;margin:8px 0;
  box-shadow:0 8px 32px rgba(0,0,0,.4);
}
.md-code-hdr{
  padding:7px 12px;background:rgba(12,10,24,.8);border-bottom:1px solid var(--bd);
  display:flex;align-items:center;justify-content:space-between;gap:8px;
}
.md-code-lang{font-size:9px;color:var(--ac);font-family:var(--font-mono);font-weight:600;letter-spacing:.5px;text-transform:uppercase}
.md-code-copy{
  font-size:8px;padding:3px 8px;background:rgba(157,78,255,.1);border:1px solid rgba(157,78,255,.25);
  border-radius:4px;color:var(--ac);cursor:pointer;font-family:var(--font-mono);transition:all .15s;
}
.md-code-copy:hover{background:rgba(157,78,255,.2);border-color:var(--ac)}
.md-code-copy.copied{background:rgba(0,255,136,.12);border-color:rgba(0,255,136,.3);color:var(--green)}
.md-code-body{padding:12px 14px;overflow-x:auto;font-family:var(--font-mono);font-size:11px;line-height:1.7;color:#D4C8FF}
.md-ic{background:rgba(157,78,255,.1);border:1px solid rgba(157,78,255,.2);padding:1px 5px;border-radius:4px;font-family:var(--font-mono);font-size:.9em;color:var(--ac)}
.md-link{color:var(--cyan);text-decoration:underline;text-decoration-color:rgba(0,212,255,.3);transition:color .15s}
.md-link:hover{color:var(--ac);text-decoration-color:var(--ac)}
.md-h1,.md-h2,.md-h3{font-family:var(--font-display);font-weight:700;margin:10px 0 6px;background:linear-gradient(135deg,var(--tx),rgba(200,180,255,.8));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.md-h1{font-size:18px}.md-h2{font-size:15px}.md-h3{font-size:13px}
.md-p{margin:3px 0;line-height:1.72}
.md-hr{border:none;border-top:1px solid var(--bd);margin:10px 0}
.md-bq{border-left:2px solid var(--ac);padding:4px 12px;color:var(--mu);background:rgba(157,78,255,.04);border-radius:0 6px 6px 0;margin:6px 0}
.md-list{padding-left:18px;margin:4px 0}
.md-list li{margin:2px 0;line-height:1.65}

/* ══ COT (Chain of Thought) ══ */
.cot-block{
  background:rgba(157,78,255,.04);border:1px solid rgba(157,78,255,.15);
  border-radius:8px;margin:6px 0;overflow:hidden;
}
.cot-toggle{
  width:100%;padding:7px 12px;background:transparent;border:none;
  display:flex;align-items:center;gap:8px;cursor:pointer;
  font-size:10px;color:var(--mu);font-family:var(--font-mono);
  transition:background .15s;
}
.cot-toggle:hover{background:rgba(157,78,255,.06)}
.cot-body{padding:10px 14px;font-size:10px;color:var(--mu);line-height:1.7;font-family:var(--font-mono);border-top:1px solid var(--bd)}

/* ══ STATUS ══ */
.status-cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(90px,1fr));gap:6px}
.sc{padding:8px 10px;border-radius:8px;border:1px solid}
input[type=file]{display:none}

/* ══ CLASSES MANQUANTES ══ */
.persona-bar{display:flex;align-items:center;gap:8px;padding:5px clamp(10px,2vw,16px);border-bottom:1px solid var(--bd);background:rgba(5,3,8,.6);flex-shrink:0;overflow-x:auto;scrollbar-width:none}
.persona-bar::-webkit-scrollbar{display:none}
.persona-lbl{font-size:9px;color:var(--mu);font-family:var(--font-mono);letter-spacing:1px;white-space:nowrap;flex-shrink:0}
.persona-chips{display:flex;gap:4px;flex-wrap:nowrap}
.persona-chip{padding:3px 10px;border-radius:5px;border:1px solid var(--bd);font-size:9px;cursor:pointer;background:transparent;color:var(--mu);font-family:var(--font-mono);white-space:nowrap;transition:all .15s;flex-shrink:0}
.persona-chip:hover{border-color:var(--bd2);color:var(--tx)}
.persona-chip.on{font-weight:600}
.persona-add{padding:3px 10px;border-radius:5px;border:1px dashed rgba(157,78,255,.3);font-size:9px;cursor:pointer;background:transparent;color:var(--ac);font-family:var(--font-mono);white-space:nowrap;transition:all .15s;flex-shrink:0}
.persona-add:hover{border-color:var(--ac);background:rgba(157,78,255,.06)}
.zen-mode-btn{background:rgba(12,10,24,.7);border:1px solid var(--bd);color:var(--mu);border-radius:7px;width:30px;height:30px;cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center;transition:all .2s;flex-shrink:0;padding:0}
.zen-mode-btn:hover{border-color:var(--bd2);color:var(--tx);background:rgba(157,78,255,.08)}
.canvas-panel{flex:1;display:flex;flex-direction:column;overflow:hidden;background:rgba(5,3,8,.8)}
.canvas-hdr{padding:8px 14px;border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:8px;flex-shrink:0;background:rgba(12,10,24,.7)}
.canvas-iframe{flex:1;border:none;background:#fff;width:100%;height:100%}
.media-subtabs{display:flex;gap:3px;padding:6px 14px;border-bottom:1px solid var(--bd);flex-shrink:0;background:rgba(5,3,8,.6);overflow-x:auto;scrollbar-width:none}
.media-content{flex:1;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.attach-preview{display:flex;align-items:center;gap:8px;padding:6px 10px;background:rgba(157,78,255,.06);border:1px solid rgba(157,78,255,.2);border-radius:8px;font-size:10px;margin-bottom:6px}
.offline-bar{padding:5px 14px;background:rgba(255,77,106,.08);border-bottom:1px solid rgba(255,77,106,.2);font-size:9px;color:var(--red);text-align:center;font-family:var(--font-mono);flex-shrink:0}
.pvar-chip{padding:2px 8px;border-radius:4px;background:rgba(0,212,255,.08);border:1px solid rgba(0,212,255,.2);color:var(--cyan);font-size:8px;font-family:var(--font-mono);cursor:pointer;transition:all .15s}
.pvar-chip:hover{background:rgba(0,212,255,.15)}
.pvar-hint{font-size:8px;color:var(--mu);font-family:var(--font-mono);margin-top:3px}
.pwa-banner{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);z-index:500;background:rgba(12,10,24,.97);border:1px solid var(--bd2);border-radius:14px;padding:14px 18px;display:flex;align-items:center;gap:12px;max-width:340px;width:calc(100vw - 32px);box-shadow:0 20px 60px rgba(0,0,0,.6);backdrop-filter:blur(20px)}
.pwa-banner-icon{font-size:28px;flex-shrink:0}
.pwa-banner-text{flex:1}
.pwa-banner-title{font-family:var(--font-display);font-size:12px;font-weight:700;margin-bottom:2px}
.pwa-banner-sub{font-size:10px;color:var(--mu)}
.pwa-install-btn{background:linear-gradient(135deg,var(--ac2),var(--pink));color:#fff;border:none;border-radius:7px;padding:7px 14px;font-size:10px;font-family:var(--font-mono);font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0}
.pwa-dismiss-btn{background:transparent;border:none;color:var(--mu);cursor:pointer;font-size:16px;padding:2px 4px;flex-shrink:0}
.yt-add-modal{position:fixed;inset:0;background:rgba(0,0,0,.8);display:flex;align-items:center;justify-content:center;z-index:400;backdrop-filter:blur(8px)}
.yt-add-modal-box{background:rgba(12,10,24,.97);border:1px solid var(--bd2);border-radius:14px;padding:24px;width:100%;max-width:380px;box-shadow:0 40px 100px rgba(0,0,0,.6)}
.yt-add-modal-title{font-family:var(--font-display);font-size:15px;font-weight:700;margin-bottom:16px}
.yt-add-row{display:flex;flex-direction:column;gap:4px;margin-bottom:12px}
.yt-add-label{font-size:9px;color:var(--mu);font-family:var(--font-mono);letter-spacing:.5px;text-transform:uppercase}
.yt-add-inp{background:rgba(5,3,8,.8);border:1px solid var(--bd);border-radius:6px;color:var(--tx);font-family:var(--font-mono);font-size:11px;padding:7px 10px;outline:none;width:100%;transition:border-color .2s}
.yt-add-inp:focus{border-color:rgba(157,78,255,.5)}
.yt-add-field{background:rgba(5,3,8,.8);border:1px solid var(--bd);border-radius:6px;color:var(--tx);font-family:var(--font-mono);font-size:11px;padding:7px 10px;outline:none;width:100%}
.yt-add-colors{display:flex;gap:6px;flex-wrap:wrap}
.show{display:block!important}
.md{line-height:1.72}

/* ══ TOAST ══ */
.toast{
  position:fixed;bottom:clamp(12px,2.5vw,20px);right:clamp(12px,2.5vw,20px);
  background:rgba(12,10,24,.95);border:1px solid var(--bd2);border-radius:10px;
  padding:8px 14px;font-size:11px;color:var(--tx);z-index:999;
  animation:toastIn .3s ease;
  display:flex;align-items:center;gap:8px;max-width:300px;
  backdrop-filter:blur(16px);box-shadow:0 16px 48px rgba(0,0,0,.5),0 0 16px rgba(157,78,255,.1);
}
@keyframes toastIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}

/* ══ MODAL / OVERLAY ══ */
.ov{position:fixed;inset:0;background:rgba(0,0,0,.8);display:flex;align-items:center;justify-content:center;z-index:300;backdrop-filter:blur(8px);padding:16px}
.modal{
  background:rgba(12,10,24,.97);border:1px solid var(--bd2);border-radius:14px;
  padding:clamp(16px,2.5vw,24px);width:100%;max-width:380px;
  box-shadow:0 40px 100px rgba(0,0,0,.6),0 0 40px rgba(157,78,255,.08);
  backdrop-filter:blur(20px);
  animation:modalIn .25s cubic-bezier(.4,0,.2,1) both;
}
@keyframes modalIn{from{opacity:0;transform:scale(.96) translateY(8px)}to{opacity:1;transform:scale(1) translateY(0)}}
.modal h3{font-family:var(--font-display);font-size:clamp(13px,2.2vw,16px);font-weight:700;margin-bottom:5px}
.modal p{font-size:clamp(10px,1.6vw,11px);color:var(--mu);margin-bottom:12px;line-height:1.65}
.modal a{color:var(--ac);text-decoration:none}
.modal input{
  width:100%;background:rgba(5,3,8,.8);border:1px solid var(--bd);border-radius:7px;
  color:var(--tx);font-family:var(--font-mono);font-size:12px;padding:9px 11px;outline:none;margin-bottom:12px;
  transition:border-color .2s,box-shadow .2s;
}
.modal input:focus{border-color:rgba(157,78,255,.5);box-shadow:0 0 0 3px rgba(157,78,255,.08)}
.mbtns{display:flex;gap:8px;justify-content:flex-end}
.mbg{background:transparent;border:1px solid var(--bd);color:var(--mu);padding:6px 12px;border-radius:6px;font-family:var(--font-mono);font-size:9px;cursor:pointer;transition:all .15s}
.mbg:hover{border-color:var(--bd2);color:var(--tx)}
.mbs{background:linear-gradient(135deg,var(--ac2),var(--pink));border:none;color:#fff;padding:6px 12px;border-radius:6px;font-family:var(--font-mono);font-size:9px;font-weight:600;cursor:pointer;transition:all .15s}
.mbs:disabled{opacity:.3;cursor:not-allowed}
.exp-json{width:100%;background:rgba(5,3,8,.8);border:1px solid var(--bd);border-radius:7px;color:var(--green);font-family:var(--font-mono);font-size:9px;padding:9px;resize:none;outline:none;height:130px;margin-bottom:10px}
.copy-btn{background:rgba(0,255,136,.12);color:var(--green);border:1px solid rgba(0,255,136,.25);padding:5px 12px;border-radius:5px;font-family:var(--font-mono);font-size:9px;font-weight:600;cursor:pointer;transition:all .15s}
.copy-btn:hover{background:rgba(0,255,136,.2)}

/* ══ ARENA ══ */
.arena-wrap{flex:1;overflow-y:auto;padding:clamp(12px,2vw,18px);scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.arena-hero{text-align:center;padding:clamp(16px,3vw,28px) 0 clamp(12px,2vw,20px);margin-bottom:clamp(12px,2vw,20px);border-bottom:1px solid var(--bd)}
.arena-title{font-family:var(--font-display);font-size:clamp(20px,3.5vw,28px);font-weight:900;background:linear-gradient(135deg,var(--ac),var(--pink),var(--cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px}
.arena-sub{font-size:clamp(10px,1.6vw,12px);color:var(--mu);line-height:1.7;max-width:600px;margin:0 auto}
.criteria-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(clamp(130px,18vw,190px),1fr));gap:10px;margin-bottom:clamp(14px,2.5vw,22px)}
.crit-card{background:rgba(12,10,24,.7);border:1px solid var(--bd);border-radius:10px;padding:12px 14px;transition:all .25s}
.crit-card:hover{border-color:var(--bd2);transform:translateY(-2px)}
.crit-icon{font-size:20px;margin-bottom:6px}
.crit-title{font-family:var(--font-display);font-size:11px;font-weight:700;color:var(--tx);margin-bottom:3px}
.crit-desc{font-size:9px;color:var(--mu);line-height:1.5}
.arena-tbl{width:100%;border-collapse:collapse;border:1px solid var(--bd);border-radius:10px;overflow:hidden;min-width:700px}
.arena-tbl th{padding:7px 11px;background:rgba(12,10,24,.8);font-size:8px;color:var(--mu);text-align:left;font-weight:600;letter-spacing:.7px;border-bottom:1px solid var(--bd);font-family:var(--font-mono);text-transform:uppercase}
.arena-tbl td{padding:9px 11px;border-bottom:1px solid var(--bd);font-size:clamp(9px,1.5vw,11px);vertical-align:middle}
.arena-tbl tr:last-child td{border-bottom:none}
.arena-tbl tr:hover td{background:rgba(157,78,255,.03)}
.score-bar{display:flex;align-items:center;gap:6px}
.score-num{font-weight:700;font-size:13px;min-width:30px;font-family:var(--font-mono)}
.score-bg{flex:1;height:4px;background:rgba(157,78,255,.1);border-radius:2px;overflow:hidden;min-width:40px}
.score-fill{height:100%;border-radius:2px;box-shadow:0 0 6px currentColor}
.tag-badge{display:inline-flex;padding:1px 6px;border-radius:4px;font-size:8px;font-weight:700;letter-spacing:.5px;background:rgba(157,78,255,.1);color:var(--ac);border:1px solid rgba(157,78,255,.2)}
.prix-badge{font-family:var(--font-mono);font-size:10px}
.params-badge{font-family:var(--font-mono);font-size:9px;color:var(--cyan)}

/* ══ NEWS / RSS ══ */
.news-grid{display:flex;flex-direction:column;gap:10px;margin-top:12px}
.rss-wrap{background:rgba(12,10,24,.7);border:1px solid var(--bd);border-radius:12px;overflow:hidden;margin-bottom:24px;backdrop-filter:blur(8px)}
.rss-head{padding:12px 16px;background:rgba(5,3,8,.6);border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.rss-title{font-family:var(--font-display);font-size:14px;font-weight:800;color:var(--green)}
.rss-src-tabs{display:flex;gap:4px;flex-wrap:wrap;flex:1}
.rss-stab{padding:3px 9px;border-radius:4px;font-size:8px;font-family:var(--font-mono);cursor:pointer;border:1px solid var(--bd);background:transparent;color:var(--mu);transition:all .15s}
.rss-stab.on{background:rgba(0,255,136,.1);color:var(--green);border-color:rgba(0,255,136,.25);font-weight:700}
.rss-stab:hover{border-color:rgba(0,255,136,.2);color:var(--green)}
.rss-refresh{font-size:9px;color:var(--mu);cursor:pointer;padding:3px 8px;border:1px solid var(--bd);border-radius:4px;background:transparent;font-family:var(--font-mono);transition:all .15s}
.rss-refresh:hover{color:var(--green);border-color:rgba(0,255,136,.25)}
.rss-loading{padding:24px;text-align:center;color:var(--mu);font-size:10px;font-style:italic}
.rss-err{padding:14px 16px;color:var(--orange);font-size:10px;background:rgba(255,140,66,.06)}
.rss-list{display:flex;flex-direction:column;gap:0}
.rss-item{display:flex;flex-direction:column;padding:14px 16px;border-bottom:1px solid var(--bd);text-decoration:none;transition:background .15s;cursor:pointer}
.rss-item:last-child{border-bottom:none}
.rss-item:hover{background:rgba(157,78,255,.04)}
.rss-item-hdr{display:flex;gap:9px;align-items:flex-start}
...
.rss-site{font-size:8px;font-weight:700;padding:1px 6px;border-radius:3px}
.rss-date{font-size:8px;color:var(--mu)}
.rss-summary{font-size:clamp(9px,1.4vw,10px);color:var(--mu);line-height:1.6;margin-top:6px}
.rss-expand-body{margin-top:10px;padding-top:10px;border-top:1px solid var(--bd);display:flex;flex-direction:column;gap:8px}
.rss-detail{font-size:clamp(9px,1.4vw,10px);color:rgba(200,190,240,.7);line-height:1.6}
.rss-impact{font-size:clamp(8px,1.3vw,9px);color:var(--green);padding:6px 10px;background:rgba(0,255,136,.05);border-radius:5px;border-left:2px solid var(--green)}
.rss-expand-footer{display:flex;align-items:center;gap:9px;margin-top:4px;flex-wrap:wrap}
.rss-gnews-btn{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;background:rgba(157,78,255,.08);border:1px solid rgba(157,78,255,.2);border-radius:5px;color:var(--ac);font-size:9px;font-weight:600;text-decoration:none;font-family:var(--font-mono);transition:all .2s}
.rss-gnews-btn:hover{background:rgba(157,78,255,.16)}
.rss-toggle{margin-left:auto;background:none;border:none;color:var(--mu);cursor:pointer;font-size:9px;font-family:var(--font-mono);padding:2px 7px;border-radius:4px;transition:color .15s}
.rss-toggle:hover{color:var(--tx)}
.rss-arrow{font-size:12px;color:var(--mu);flex-shrink:0;margin-top:1px;transition:color .2s}
...
/* ══ NEWS CARD ══ */
.news-card{background:rgba(12,10,24,.7);border:1px solid var(--bd);border-radius:10px;padding:12px 15px;display:flex;gap:12px;transition:all .25s}
.news-card:hover{border-color:var(--bd2);transform:translateY(-2px)}
.news-icon{font-size:22px;flex-shrink:0;margin-top:2px}
.news-body{flex:1}
.news-hdr{display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap}
.news-title{font-family:var(--font-display);font-size:clamp(10px,1.8vw,12px);font-weight:700}
.news-date{font-size:8px;color:var(--mu)}
.news-tag{padding:1px 6px;border-radius:3px;font-size:7px;font-weight:700;letter-spacing:.5px}
.news-text{font-size:clamp(9px,1.5vw,11px);color:var(--mu);line-height:1.6}

/* ══ IMAGES ══ */
.img-wrap{flex:1;overflow-y:auto;padding:clamp(12px,2vw,18px);scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.img-filter{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:clamp(12px,2vw,18px)}
.filter-btn{padding:5px 12px;border-radius:6px;font-size:9px;font-family:var(--font-ui);cursor:pointer;border:1px solid var(--bd);background:transparent;color:var(--mu);transition:all .2s}
.filter-btn.on{background:linear-gradient(135deg,rgba(123,47,255,.2),rgba(255,60,172,.1));color:var(--tx);border-color:rgba(157,78,255,.35);font-weight:600}
.filter-btn:hover{border-color:var(--bd2);color:var(--tx)}
.img-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(clamp(230px,28vw,310px),1fr));gap:clamp(10px,1.5vw,14px)}
.img-card{background:rgba(12,10,24,.7);border:1px solid var(--bd);border-radius:12px;padding:clamp(14px,2vw,18px);display:flex;flex-direction:column;gap:12px;transition:all .25s;backdrop-filter:blur(6px)}
.img-card:hover{transform:translateY(-3px);border-color:var(--bd2);box-shadow:0 20px 60px rgba(0,0,0,.4),0 0 20px rgba(157,78,255,.06)}
.img-card.oss{border-color:rgba(0,255,136,.15)}
.ic-hdr{display:flex;align-items:center;gap:10px}
.ic-icon{width:clamp(32px,4vw,42px);height:clamp(32px,4vw,42px);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:clamp(15px,2.2vw,20px);flex-shrink:0}
.ic-name{font-family:var(--font-display);font-weight:700;font-size:clamp(12px,1.9vw,14px)}
.ic-sub{font-size:8px;color:var(--mu)}
.ic-free{display:inline-flex;padding:2px 8px;border-radius:5px;font-size:8px;font-weight:700}
.ic-meters{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}
.meter-item{text-align:center}
.meter-lbl{font-size:7px;color:var(--mu);margin-bottom:3px}
.meter-bg{height:3px;background:rgba(157,78,255,.1);border-radius:2px;overflow:hidden}
.meter-fill{height:100%;border-radius:2px;box-shadow:0 0 4px currentColor}
...
.ic-str{padding:2px 7px;border-radius:4px;font-size:8px;background:rgba(157,78,255,.06);border:1px solid var(--bd);color:var(--mu)}
.ic-limits{font-size:8px;color:var(--orange);padding:5px 8px;background:rgba(255,140,66,.06);border-radius:5px;border:1px solid rgba(255,140,66,.15)}
.ic-btn{display:flex;align-items:center;justify-content:center;gap:5px;padding:9px;border-radius:7px;border:1px solid;font-family:var(--font-mono);font-size:9px;font-weight:600;cursor:pointer;background:transparent;text-decoration:none;transition:all .2s}
.ic-btn:hover{opacity:.8;transform:scale(1.01)}
.ic-tags{display:flex;flex-wrap:wrap;gap:3px}
.ic-tag{padding:1px 6px;border-radius:3px;font-size:7px;background:rgba(255,255,255,.03);border:1px solid var(--bd);color:var(--mu)}

/* ══ YOUTUBE ══ */
.yt-wrap{flex:1;overflow-y:auto;padding:clamp(12px,2vw,18px);scrollbar-width:thin;scrollbar-color:var(--bd) transparent}
.yt-hero{display:flex;align-items:center;gap:14px;padding:clamp(14px,2vw,20px);background:linear-gradient(135deg,rgba(20,5,10,.8),rgba(12,10,24,.8));border:1px solid rgba(255,60,172,.15);border-radius:12px;margin-bottom:clamp(14px,2vw,20px)}
.yt-hero-icon{font-size:clamp(30px,5vw,44px);flex-shrink:0}
.yt-hero-title{font-family:var(--font-display);font-size:clamp(18px,3vw,24px);font-weight:900;color:#FF3CAC;margin-bottom:4px}
.yt-hero-sub{font-size:clamp(10px,1.5vw,12px);color:var(--mu);line-height:1.6}
.yt-search-bar{display:flex;gap:9px;margin-bottom:clamp(12px,2vw,18px)}
.yt-search-inp{flex:1;background:rgba(12,10,24,.8);border:1px solid var(--bd);border-radius:8px;color:var(--tx);font-family:var(--font-mono);font-size:clamp(11px,1.8vw,13px);padding:9px 13px;outline:none;transition:border-color .2s,box-shadow .2s}
.yt-search-inp:focus{border-color:rgba(255,60,172,.4);box-shadow:0 0 0 3px rgba(255,60,172,.06)}
.yt-search-btn{background:linear-gradient(135deg,#CC0000,#FF0000);color:#fff;border:none;border-radius:8px;padding:9px 18px;font-family:var(--font-mono);font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:5px;transition:all .2s}
.yt-search-btn:hover{transform:translateY(-1px);box-shadow:0 0 16px rgba(255,0,0,.3)}
.yt-sec-title{font-family:var(--font-display);font-weight:800;font-size:clamp(11px,1.8vw,13px);color:var(--tx);margin-bottom:12px;padding-bottom:6px;border-bottom:1px solid var(--bd);display:flex;align-items:center;gap:8px;letter-spacing:.5px}
.yt-ch-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(clamp(190px,22vw,250px),1fr));gap:clamp(8px,1.2vw,12px);margin-bottom:clamp(18px,3vw,28px)}

/* ══ ONBOARDING ══ */
.onboarding-overlay{position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:9999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(10px)}
.onboarding-card{
  background:rgba(12,10,24,.97);border:1px solid var(--bd2);border-radius:20px;
  padding:36px 40px;max-width:500px;width:90%;text-align:center;position:relative;
  box-shadow:0 40px 100px rgba(0,0,0,.6),0 0 60px rgba(157,78,255,.1);
  backdrop-filter:blur(20px);
}
.onboarding-step{font-size:9px;color:var(--mu);font-family:var(--font-mono);margin-bottom:10px;letter-spacing:2.5px;text-transform:uppercase}
.onboarding-title{font-family:var(--font-display);font-weight:900;font-size:24px;background:linear-gradient(135deg,var(--ac),var(--pink));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:10px}
.onboarding-desc{font-size:12px;color:var(--mu);line-height:1.75;margin-bottom:28px}
.onboarding-dots{display:flex;gap:7px;justify-content:center;margin-bottom:22px}
.onboarding-dot{width:8px;height:8px;border-radius:50%;background:var(--bd);transition:all .2s}
.onboarding-dot.on{background:var(--ac);width:22px;border-radius:4px;box-shadow:0 0 8px var(--ac)}

/* ══ GLOBAL SEARCH ══ */
.global-search-overlay{position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:8000;display:flex;align-items:flex-start;justify-content:center;padding-top:14vh;backdrop-filter:blur(6px)}
.global-search-box{
  background:rgba(12,10,24,.97);border:1px solid rgba(157,78,255,.35);
  border-radius:14px;width:90%;max-width:620px;overflow:hidden;
  box-shadow:0 30px 80px rgba(0,0,0,.6),0 0 40px rgba(157,78,255,.12);
  backdrop-filter:blur(20px);
}
.global-search-input{width:100%;background:transparent;border:none;padding:18px 22px;font-size:16px;color:var(--tx);font-family:var(--font-ui);outline:none}
.global-search-results{max-height:420px;overflow-y:auto}
.global-search-result{padding:11px 22px;cursor:pointer;border-top:1px solid var(--bd);transition:background .1s}
.global-search-result:hover{background:rgba(157,78,255,.06)}
.global-search-result-title{font-size:10px;font-weight:700;color:var(--ac);margin-bottom:3px;font-family:var(--font-mono);letter-spacing:.5px;text-transform:uppercase}
.global-search-result-excerpt{font-size:9px;color:var(--mu);line-height:1.55}
.global-search-result mark{background:rgba(157,78,255,.2);color:var(--tx);border-radius:3px;padding:0 3px}

/* ══ THEME PICKER ══ */
.theme-picker{
  position:absolute;top:calc(100% + 8px);right:0;
  background:rgba(12,10,24,.97);border:1px solid var(--bd2);
  border-radius:10px;padding:4px;z-index:300;
  display:flex;flex-direction:column;gap:2px;min-width:130px;
  box-shadow:0 16px 48px rgba(0,0,0,.5),0 0 20px rgba(157,78,255,.08);
  backdrop-filter:blur(16px);
}
.theme-option{padding:7px 14px;border-radius:6px;cursor:pointer;font-size:9px;color:var(--tx);font-family:var(--font-mono);transition:background .1s;border:none;background:transparent;text-align:left;white-space:nowrap}
.theme-option:hover{background:rgba(157,78,255,.1)}
.theme-option.on{background:rgba(157,78,255,.15);color:var(--ac);font-weight:700}

/* ══ PS CODE BLOCKS ══ */
.ps-block{background:rgba(5,3,8,.9);border:1px solid var(--bd);border-radius:10px;overflow:hidden;margin:10px 0}
.ps-hdr{padding:6px 14px;background:rgba(12,10,24,.8);border-bottom:1px solid var(--bd);display:flex;align-items:center;justify-content:space-between}
.ps-lang{font-size:9px;color:var(--ac);font-weight:700;letter-spacing:.5px;font-family:var(--font-mono);text-transform:uppercase}
.ps-copy{font-size:9px;color:var(--mu);cursor:pointer;padding:2px 7px;border:1px solid var(--bd);border-radius:4px;background:transparent;font-family:var(--font-mono);transition:all .15s}
.ps-copy:hover{color:var(--green);border-color:rgba(0,255,136,.25)}
.ps-code{padding:12px 16px;overflow-x:auto}
.ps-code pre{font-family:var(--font-mono);font-size:11px;color:#D4C8FF;line-height:1.75;white-space:pre}
.ps-comment{color:rgba(107,95,158,.7);font-style:italic}
.ps-cmd{color:var(--green)}
.ps-arg{color:var(--ac)}
.ps-str{color:var(--orange)}

/* ══ TAB TRANSITIONS ══ */
@keyframes tabEnter{from{opacity:0;transform:translateY(10px) scale(.99)}to{opacity:1;transform:translateY(0) scale(1)}}
@keyframes tabEnterLeft{from{opacity:0;transform:translateX(-20px)}to{opacity:1;transform:translateX(0)}}
@keyframes tabEnterRight{from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)}}
...
@keyframes pulse-glow{0%,100%{opacity:.5;transform:scale(1)}50%{opacity:1;transform:scale(1.3)}}
...
function App() {
  ...
  const [projects, setProjects] = useState(() => {
    try { return JSON.parse(localStorage.getItem("multiia_projects") || "[]"); } catch { return []; }
  });
  ...
  useEffect(() => {
    ...
    root.classList.remove("light","nord","dracula","noir");
...
const tabsRef2=React.useRef(null);
const scroll=(dir)=>{if(tabsRef2.current)tabsRef2.current.scrollLeft+=dir*360;};
const SVG_IMGS={
  aide:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgaide%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgaide%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3E%3F%3C/text%3E%0A%3C/svg%3E`,
  studio:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgstud%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF3CAC%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c30070%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgstud%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3ES%3C/text%3E%0A%3C/svg%3E`,
  router:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgrout%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2300D4FF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%230098c3%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgrout%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ER%3C/text%3E%0A%3C/svg%3E`,
  chat:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gdchat%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpchat%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gdchat%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpchat%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EC%3C/text%3E%0A%3C/svg%3E`,
  prompts:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wgprom%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FFB700%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c37b00%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wgprom%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EP%3C/text%3E%0A%3C/svg%3E`,
  redaction:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgreda%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF8C42%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c35006%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgreda%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ERe%3C/text%3E%0A%3C/svg%3E`,
  recherche:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22tgrech%22%20x1=%220%25%22%20y1=%22100%25%22%20x2=%22100%25%22%20y2=%220%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2300D4FF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%230098c3%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23tgrech%29%22/%3E%0A%3Cpolygon%20points=%220%2C80%2060%2C0%20120%2C80%22%20fill=%22white%22%20fill-opacity=%220.06%22/%3E%0A%3Cpolygon%20points=%2240%2C80%2090%2C10%20130%2C80%22%20fill=%22white%22%20fill-opacity=%220.05%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2242%22%20r=%2220%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ERch%3C/text%3E%0A%3C/svg%3E`,
  workflows:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22xgwork%22%20cx=%2260%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23xgwork%29%22/%3E%0A%3Cline%20x1=%220%22%20y1=%2240%22%20x2=%22130%22%20y2=%2240%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Cline%20x1=%2265%22%20y1=%220%22%20x2=%2265%22%20y2=%2280%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Crect%20x=%2222%22%20y=%2212%22%20width=%2286%22%20height=%2256%22%20rx=%228%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Crect%20x=%2232%22%20y=%2220%22%20width=%2266%22%20height=%2240%22%20rx=%225%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EW%3C/text%3E%0A%3C/svg%3E`,
  medias:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgmedi%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF3CAC%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c30070%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgmedi%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EM%3C/text%3E%0A%3C/svg%3E`,
  comfyui:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgcomf%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2300FF88%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300c34c%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgcomf%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EImg%3C/text%3E%0A%3C/svg%3E`,
  arena:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgaren%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FFB700%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c37b00%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgaren%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EA%3C/text%3E%0A%3C/svg%3E`,
  debate:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gddeba%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF4D6A%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c3112e%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpdeba%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gddeba%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpdeba%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ED%3C/text%3E%0A%3C/svg%3E`,
  expert:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wgexpe%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23A78BFA%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236b4fbe%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wgexpe%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EEx%3C/text%3E%0A%3C/svg%3E`,
  compare:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgcomp%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2300D4FF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%230098c3%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgcomp%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3E=%3C/text%3E%0A%3C/svg%3E`,
  notes:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22tgnote%22%20x1=%220%25%22%20y1=%22100%25%22%20x2=%22100%25%22%20y2=%220%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2334D399%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300975d%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23tgnote%29%22/%3E%0A%3Cpolygon%20points=%220%2C80%2060%2C0%20120%2C80%22%20fill=%22white%22%20fill-opacity=%220.06%22/%3E%0A%3Cpolygon%20points=%2240%2C80%2090%2C10%20130%2C80%22%20fill=%22white%22%20fill-opacity=%220.05%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2242%22%20r=%2220%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EN%3C/text%3E%0A%3C/svg%3E`,
  traducteur:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22xgtrad%22%20cx=%2260%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2306B6D4%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23007a98%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23xgtrad%29%22/%3E%0A%3Cline%20x1=%220%22%20y1=%2240%22%20x2=%22130%22%20y2=%2240%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Cline%20x1=%2265%22%20y1=%220%22%20x2=%2265%22%20y2=%2280%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Crect%20x=%2222%22%20y=%2212%22%20width=%2286%22%20height=%2256%22%20rx=%228%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Crect%20x=%2232%22%20y=%2220%22%20width=%2266%22%20height=%2240%22%20rx=%225%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ETr%3C/text%3E%0A%3C/svg%3E`,
  agent:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgagen%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgagen%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EAg%3C/text%3E%0A%3C/svg%3E`,
  webia:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgwebi%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2360A5FA%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%232469be%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgwebi%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EWeb%3C/text%3E%0A%3C/svg%3E`,
  stats:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgstat%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23C084FC%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%238448c0%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgstat%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ESt%3C/text%3E%0A%3C/svg%3E`,
  analytics:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gdanal%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23F97316%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23bd3700%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpanal%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gdanal%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpanal%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EAn%3C/text%3E%0A%3C/svg%3E`,
  veille:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wgveil%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2334D399%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300975d%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wgveil%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EV%3C/text%3E%0A%3C/svg%3E`,
  voice:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgvoic%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23EC4899%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23b00c5d%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgvoic%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EVo%3C/text%3E%0A%3C/svg%3E`,
  projects:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22tgproj%22%20x1=%220%25%22%20y1=%22100%25%22%20x2=%22100%25%22%20y2=%220%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2310B981%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23007d45%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23tgproj%29%22/%3E%0A%3Cpolygon%20points=%220%2C80%2060%2C0%20120%2C80%22%20fill=%22white%22%20fill-opacity=%220.06%22/%3E%0A%3Cpolygon%20points=%2240%2C80%2090%2C10%20130%2C80%22%20fill=%22white%22%20fill-opacity=%220.05%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2242%22%20r=%2220%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EPr%3C/text%3E%0A%3C/svg%3E`,
  benchmark:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22xgbenc%22%20cx=%2260%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23F59E0B%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23b96200%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23xgbenc%29%22/%3E%0A%3Cline%20x1=%220%22%20y1=%2240%22%20x2=%22130%22%20y2=%2240%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Cline%20x1=%2265%22%20y1=%220%22%20x2=%2265%22%20y2=%2280%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Crect%20x=%2222%22%20y=%2212%22%20width=%2286%22%20height=%2256%22%20rx=%228%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Crect%20x=%2232%22%20y=%2220%22%20width=%2266%22%20height=%2240%22%20rx=%225%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EBk%3C/text%3E%0A%3C/svg%3E`,
  glossaire:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgglos%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%238B5CF6%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%234f20ba%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgglos%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EG%3C/text%3E%0A%3C/svg%3E`,
  autopsy:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgauto%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23EF4444%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23b30808%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgauto%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EAu%3C/text%3E%0A%3C/svg%3E`,
  mentor:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgment%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgment%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EMe%3C/text%3E%0A%3C/svg%3E`,
  dna:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gddna%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2300FF88%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300c34c%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpdna%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gddna%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpdna%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EDNA%3C/text%3E%0A%3C/svg%3E`,
  conference:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wgconf%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2306B6D4%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23007a98%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wgconf%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ECf%3C/text%3E%0A%3C/svg%3E`,
  consensus:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgcons%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23A78BFA%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236b4fbe%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgcons%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ECo%3C/text%3E%0A%3C/svg%3E`,
  brief:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22tgbrie%22%20x1=%220%25%22%20y1=%22100%25%22%20x2=%22100%25%22%20y2=%220%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FFB700%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c37b00%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23tgbrie%29%22/%3E%0A%3Cpolygon%20points=%220%2C80%2060%2C0%20120%2C80%22%20fill=%22white%22%20fill-opacity=%220.06%22/%3E%0A%3Cpolygon%20points=%2240%2C80%2090%2C10%20130%2C80%22%20fill=%22white%22%20fill-opacity=%220.05%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2242%22%20r=%2220%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EBr%3C/text%3E%0A%3C/svg%3E`,
  taskia:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22xgtask%22%20cx=%2260%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23F97316%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23bd3700%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23xgtask%29%22/%3E%0A%3Cline%20x1=%220%22%20y1=%2240%22%20x2=%22130%22%20y2=%2240%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Cline%20x1=%2265%22%20y1=%220%22%20x2=%2265%22%20y2=%2280%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Crect%20x=%2222%22%20y=%2212%22%20width=%2286%22%20height=%2256%22%20rx=%228%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Crect%20x=%2232%22%20y=%2220%22%20width=%2266%22%20height=%2240%22%20rx=%225%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ET%3C/text%3E%0A%3C/svg%3E`,
  journaliste:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgjour%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2334D399%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300975d%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgjour%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EJn%3C/text%3E%0A%3C/svg%3E`,
  skills:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgskil%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%238B5CF6%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%234f20ba%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgskil%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3ESk%3C/text%3E%0A%3C/svg%3E`,
  contradict:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgcont%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23EF4444%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23b30808%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgcont%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EX%3C/text%3E%0A%3C/svg%3E`,
  secondbrain:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gdseco%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2310B981%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23007d45%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpseco%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gdseco%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpseco%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ESB%3C/text%3E%0A%3C/svg%3E`,
  livedebate:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wglive%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF4D6A%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c3112e%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wglive%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ELd%3C/text%3E%0A%3C/svg%3E`,
  contexttrans:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgcont%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2306B6D4%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23007a98%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgcont%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ECt%3C/text%3E%0A%3C/svg%3E`,
  apioptim:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22tgapio%22%20x1=%220%25%22%20y1=%22100%25%22%20x2=%22100%25%22%20y2=%220%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2360A5FA%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%232469be%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23tgapio%29%22/%3E%0A%3Cpolygon%20points=%220%2C80%2060%2C0%20120%2C80%22%20fill=%22white%22%20fill-opacity=%220.06%22/%3E%0A%3Cpolygon%20points=%2240%2C80%2090%2C10%20130%2C80%22%20fill=%22white%22%20fill-opacity=%220.05%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2242%22%20r=%2220%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EAPI%3C/text%3E%0A%3C/svg%3E`,
  civilisations:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22xgcivi%22%20cx=%2260%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23F59E0B%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23b96200%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23xgcivi%29%22/%3E%0A%3Cline%20x1=%220%22%20y1=%2240%22%20x2=%22130%22%20y2=%2240%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Cline%20x1=%2265%22%20y1=%220%22%20x2=%2265%22%20y2=%2280%22%20stroke=%22white%22%20opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Crect%20x=%2222%22%20y=%2212%22%20width=%2286%22%20height=%2256%22%20rx=%228%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Crect%20x=%2232%22%20y=%2220%22%20width=%2266%22%20height=%2240%22%20rx=%225%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ECv%3C/text%3E%0A%3C/svg%3E`,
  flash:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3CradialGradient%20id=%22rgflas%22%20cx=%2250%25%22%20cy=%2240%25%22%20r=%2270%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FFB700%22%20stop-opacity=%220.9%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c37b00%22%20stop-opacity=%221%22/%3E%0A%3C/radialGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23rgflas%29%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2260%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.08%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2240%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%221%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2240%22%20r=%2222%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2247%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EF%3C/text%3E%0A%3C/svg%3E`,
  promptiter:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22lgprom%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239D4EFF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%236112c3%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23lgprom%29%22/%3E%0A%3Cline%20x1=%22-10%22%20y1=%2220%22%20x2=%2290%22%20y2=%22100%22%20stroke=%22white%22%20stroke-opacity=%220.1%22%20stroke-width=%2216%22/%3E%0A%3Cline%20x1=%2220%22%20y1=%22-10%22%20x2=%22140%22%20y2=%2290%22%20stroke=%22white%22%20stroke-opacity=%220.07%22%20stroke-width=%2216%22/%3E%0A%3Crect%20x=%2225%22%20y=%2218%22%20width=%2280%22%20height=%2244%22%20rx=%228%22%20fill=%22white%22%20fill-opacity=%220.12%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2218%22%20fill=%22white%22%20opacity=%220.95%22%3EPi%3C/text%3E%0A%3C/svg%3E`,
  ltm:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22hgltm%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%2334D399%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%2300975d%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23hgltm%29%22/%3E%0A%3Cpolygon%20points=%2265%2C6%20100%2C26%20100%2C66%2065%2C86%2030%2C66%2030%2C26%22%20fill=%22none%22%20stroke=%22white%22%20stroke-opacity=%220.15%22%20stroke-width=%221.5%22/%3E%0A%3Cpolygon%20points=%2265%2C18%2088%2C31%2088%2C57%2065%2C70%2042%2C57%2042%2C31%22%20fill=%22white%22%20fill-opacity=%220.1%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2248%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ELTM%3C/text%3E%0A%3C/svg%3E`,
  imageflux:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%0A%3CradialGradient%20id=%22gdimag%22%20cx=%2230%25%22%20cy=%2230%25%22%20r=%2280%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%23FF3CAC%22%20stop-opacity=%220.95%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23c30070%22/%3E%0A%3C/radialGradient%3E%0A%3Cpattern%20id=%22dpimag%22%20width=%2214%22%20height=%2214%22%20patternUnits=%22userSpaceOnUse%22%3E%0A%3Ccircle%20cx=%227%22%20cy=%227%22%20r=%221.2%22%20fill=%22white%22%20opacity=%220.2%22/%3E%0A%3C/pattern%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23gdimag%29%22/%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23dpimag%29%22/%3E%0A%3Crect%20x=%2230%22%20y=%2220%22%20width=%2270%22%20height=%2240%22%20rx=%226%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EFx%3C/text%3E%0A%3C/svg%3E`,
  advanced:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22wgadva%22%20x1=%220%25%22%20y1=%220%25%22%20x2=%22100%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%239CA3AF%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%23606773%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23wgadva%29%22/%3E%0A%3Cpath%20d=%22M0%2C55%20Q32%2C30%2065%2C55%20T130%2C55%20L130%2C80%20L0%2C80%20Z%22%20fill=%22white%22%20opacity=%220.07%22/%3E%0A%3Cpath%20d=%22M0%2C42%20Q32%2C17%2065%2C42%20T130%2C42%22%20fill=%22none%22%20stroke=%22white%22%20opacity=%220.15%22%20stroke-width=%222%22/%3E%0A%3Ccircle%20cx=%2265%22%20cy=%2235%22%20r=%2218%22%20fill=%22white%22%20fill-opacity=%220.14%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2241%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3EAdv%3C/text%3E%0A%3C/svg%3E`,
  config:`data:image/svg+xml,%3Csvg%20xmlns=%22http%3A//www.w3.org/2000/svg%22%20viewBox=%220%200%20130%2080%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id=%22cgconf%22%20x1=%22100%25%22%20y1=%220%25%22%20x2=%220%25%22%20y2=%22100%25%22%3E%0A%3Cstop%20offset=%220%25%22%20stop-color=%22%236B7280%22/%3E%0A%3Cstop%20offset=%22100%25%22%20stop-color=%22%232f3644%22/%3E%0A%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20width=%22130%22%20height=%2280%22%20fill=%22url%28%23cgconf%29%22/%3E%0A%3Crect%20x=%220%22%20y=%220%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%22100%22%20y=%2250%22%20width=%2230%22%20height=%2230%22%20fill=%22white%22%20fill-opacity=%220.08%22%20rx=%220%22/%3E%0A%3Crect%20x=%2228%22%20y=%2216%22%20width=%2274%22%20height=%2248%22%20rx=%2210%22%20fill=%22white%22%20fill-opacity=%220.13%22/%3E%0A%3Ctext%20x=%2265%22%20y=%2245%22%20text-anchor=%22middle%22%20font-family=%22system-ui%2Csans-serif%22%20font-weight=%22800%22%20font-size=%2217%22%20fill=%22white%22%20opacity=%220.95%22%3ECfg%3C/text%3E%0A%3C/svg%3E`
};
const TD2=[["aide","#9D4EFF","Aide","Support","Guide et astuces"],["studio","#FF3CAC","Studio Auto","Studio","Tutoriels vidéo auto"],["router","#00D4FF","Router","IA","Aiguille la meilleure IA"],["chat","#9D4EFF","Chat","Essentiel","10 IAs en parallèle"],["prompts","#FFB700","Prompts","Outils","Bibliothèque de prompts"],["redaction","#FF8C42","Rédaction","Texte","Réécriture et correction"],["recherche","#00D4FF","Recherche","Analyse","Analyse et Q&A"],["workflows","#9D4EFF","Workflows","Flux","Chaîne de prompts"],["medias","#FF3CAC","Médias","Médias","YouTube IA et découverte"],["comfyui","#00FF88","ComfyUI","Images","Images via ComfyUI local"],["arena","#FFB700","Arène","Stats","Comparatif 18 modèles"],["debate","#FF4D6A","Débat","Analyse","Analyse multi-angles"],["expert","#A78BFA","Experts","Panel","Panel d'experts IA"],["compare","#00D4FF","Comparer","Outils","Diff et Jury automatique"],["notes","#34D399","Notes","Notes","Notes persistantes"],["traducteur","#06B6D4","Trad.","Texte","Traduction contextuelle"],["agent","#9D4EFF","Agent","Agent","Agent autonome multi-IA"],["webia","#60A5FA","IAs Web","Web","37 interfaces IA directes"],["stats","#C084FC","Stats","Stats","Statistiques d'usage"],["analytics","#F97316","Analytics","Stats","Analyse approfondie"],["veille","#34D399","Veille","Infos","Actualités IA temps réel"],["voice","#EC4899","Voice","Audio","Synthèse et dictée vocale"],["projects","#10B981","Projets","Travail","Gestion de projets IA"],["benchmark","#F59E0B","Bench","Stats","Tests de performance"],["glossaire","#8B5CF6","Glossaire","Réf.","Glossaire IA complet"],["autopsy","#EF4444","Autopsy","Analyse","Autopsie de prompts"],["mentor","#9D4EFF","Mentor","Coach","Coaching IA"],["dna","#00FF88","DNA","Prompts","Arbre généalogique prompts"],["conference","#06B6D4","Conférence","IA","Conférence multi-IA"],["consensus","#A78BFA","Consensus","IA","Consensus entre IAs"],["brief","#FFB700","Brief","Matin","Briefing matinal IA"],["taskia","#F97316","Task→IAs","Tâches","Délégation de tâches"],["journaliste","#34D399","Journaliste","Rédac.","Rédaction journalistique"],["skills","#8B5CF6","Skills","Appren.","Apprentissage guidé"],["contradict","#EF4444","Contradict","Débat","Débat contradictoire"],["secondbrain","#10B981","2nd Brain","Mémoire","Mémoire long terme"],["livedebate","#FF4D6A","Débat Live","Timer","Timer débat live"],["contexttrans","#06B6D4","Contexte","Traduction","Traduction contexte"],["apioptim","#60A5FA","API Optim","Dev","Optimisation prompts API"],["civilisations","#F59E0B","Civilisations","Jeu","Jeu civilisations IA"],["flash","#FFB700","Flash","Quiz","Quiz et mémorisation"],["promptiter","#9D4EFF","Itérateur","Itérat.","Itération de prompts"],["ltm","#34D399","Mémoire LT","LT Memory","Mémoire persistante LT"],["imageflux","#FF3CAC","Flux.1","Images","Génération Flux.1"],["advanced","#9CA3AF","Avancé","Avancé","Paramètres avancés"],["config","#6B7280","Config","Config","Clés API et config"]];
return(<div className="nav-tabs-wrap"><button className="nav-tabs-arrow" onClick={()=>scroll(-1)}>&#8249;</button><div className="nav-tabs" ref={tabsRef2}>{TD2.map(([t,color,label,cat,desc])=>(<button key={t} className={"nt "+(tab===t?"on":"")} onClick={()=>navigateTab(t)} style={tab===t?{borderColor:color,boxShadow:"0 0 20px "+color+"55"}:{}}><div className="nt-img" style={{backgroundImage:"url("+SVG_IMGS[t]+")",backgroundSize:"cover",backgroundPosition:"center"}}></div><div className="nt-body"><span className="nt-name">{label}</span><span className="nt-cat">{cat}</span><span className="nt-desc">{desc}</span></div></button>))}</div><button className="nav-tabs-arrow right" onClick={()=>scroll(1)}>&#8250;</button></div>);
})()}
</div>{/* fin nav-row2 */}
          <div className="pills">
            {IDS.map(id => {
